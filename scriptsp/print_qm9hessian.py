...
def format_metric_markdown(value, decimals: int, bold: bool) -> str:
    text = "-" if value is None else f"{value:.{decimals}f}"
    if bold:
        return f"**{text}**"
    return text
...
for solvent, h_label, model_name, method, is_e3x in target_rows:
    ...
    if is_e3x:
        ...
        if run is None:
            raise RuntimeError(
                f"No run found for model '{model_name}', solvent '{solvent}', method '{method}'"
            )
...
for record in df.to_dict("records"):
    ...
    line = (
        " & ".join([first_col, record["Model"], record["Hessian"], *metrics_text])
        + " \\\\"
    )
    print(line)
...
print()
print("| Solvent | Model | Hessian | Hessian ↓ |")
print("|---------|-------|---------|-----------|")
group_progress = {label: 0 for label in group_counts}
for record in df.to_dict("records"):
    solvent = record["Solvent"]
    ...
    val = record.get("hessian_mae")
    hessian_text = format_metric_markdown(val, 3, record["bold"])

    line = (
        "| "
        + " | ".join([first_col, record["Model"], record["Hessian"], hessian_text])
        + " |"
    )
    print(line)
    group_progress[solvent] += 1
