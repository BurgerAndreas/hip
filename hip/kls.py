...
class KLSWithAuxAdam(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        for group in param_groups:
            ...
            if group["use_kls"]:
                ...
                assert set(group.keys()) == set(
                    ["params", "lr", "betas", "eps", "weight_decay", "use_kls"]
                )

        super().__init__(param_groups, dict())
        print(
            "___________________________________KLS org v0___________________________________"
        )
    ...
    def update_S(
        ...
        if damping > 0:
            ...
            state["S"][idx].mul_(beta_keep).add_(
                mat
                + (trace * factor)
                * torch.eye(state["S"][idx].shape[0], device=state["S"][idx].device),
                alpha=beta_upd / factor,
            )
        else:
    ...
    def update_eigen_value(
        self, state, diag, idx, beta_keep, beta_upd, traces, total_trace, damping
    ):
        if damping > 0:
        ...
        state["eigen_sqrt_inv"][idx] = (1.0 / torch.sqrt(D)).nan_to_num_(
            nan=0.0, posinf=0.0, neginf=0.0
        )

    @torch.compile
    def update_4d_preconditioner(
        ...
        G = grad
        beta_keep_GG, beta_upd_GG = self._get_ema_weights(
            state, state["shampoo_beta"], "S"
        )
        beta_keep_Ev, beta_upd_Ev = self._get_ema_weights(
            state, state["shampoo_beta"], "eigenvalues"
        )

        GinvS1h = torch.einsum("ijab,ip->pjab", G, invS_half[1 - 1])  # G invS1
        GinvS12h = torch.einsum(
            "pjab,jq->pqab", GinvS1h, invS_half[2 - 1]
        )  # G invS1 invS2

        GinvS12Q3 = torch.einsum(
            "pqab,al->pqlb", GinvS12h, state["Q"][3 - 1]
        )  # G invS1 invS2 Q3
        GinvS12Q3Q4 = torch.einsum(
            "pqlb,bm->pqlm", GinvS12Q3, state["Q"][4 - 1]
        )  # G invS1 invS2 Q3 Q4

        GinvS123h = GinvS12Q3 * state["eigen_sqrt_inv"][3 - 1].view(
            1, 1, -1, 1
        )  # G invS1 invS2 invS3
        GinvS123G_T = torch.tensordot(
            GinvS123h, GinvS123h, dims=[[0, 1, 2], [0, 1, 2]]
        )  # for S4
        self.update_S(
            G,
            state,
        ...
        GinvS12Q4 = torch.einsum(
            "pqab,bm->pqam", GinvS12h, state["Q"][4 - 1]
        )  # G invS1 invS2 Q4
        GinvS124h = GinvS12Q4 * state["eigen_sqrt_inv"][4 - 1].view(
            1, 1, 1, -1
        )  # G invS1 invS2 invS4
        GinvS124G_T = torch.tensordot(
            GinvS124h, GinvS124h, dims=[[0, 1, 3], [0, 1, 3]]
        )  # for S3
        self.update_S(
            G,
            state,
        ...
        diag4 = torch.mean(
            (GinvS12Q3Q4 * state["eigen_sqrt_inv"][3 - 1].view(1, 1, -1, 1)) ** 2,
            dim=(0, 1, 2),
        )  # torch.diag( Q4.T @ GinvS123G_T @ Q4 )/(d1*d2*d3) #for diag(S4)
        self.update_eigen_value(
            state, diag4, 4 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )

        diag3 = torch.mean(
            (GinvS12Q3Q4 * state["eigen_sqrt_inv"][4 - 1].view(1, 1, 1, -1)) ** 2,
            dim=(0, 1, 3),
        )  # torch.diag( Q3.T @ GinvS124G_T @ Q3 )/(d1*d2*d4) #for diag(S3)
        self.update_eigen_value(
            state, diag3, 3 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )

        GinvS4h = torch.einsum("ijab,bm->ijam", G, invS_half[4 - 1])  # G invS4
        GinvS43h = torch.einsum(
            "ijam,al->ijlm", GinvS4h, invS_half[3 - 1]
        )  # G invS4 invS3

        GinvS43Q2 = torch.einsum("ijlm,jq->iqlm", GinvS43h, state["Q"][2 - 1])
        GinvS43Q2Q1 = torch.einsum(
            "iqlm,ip->pqlm", GinvS43Q2, state["Q"][1 - 1]
        )  # G invS4 invS3 Q2 Q1

        GinvS432h = GinvS43Q2 * state["eigen_sqrt_inv"][2 - 1].view(
            1, -1, 1, 1
        )  # G invS4 invS3 invS2
        GinvS432G_T = torch.tensordot(
            GinvS432h, GinvS432h, dims=[[1, 2, 3], [1, 2, 3]]
        )  # for S1
        self.update_S(
            G,
            state,
        ...
        GinvS431h = GinvS43Q1 * state["eigen_sqrt_inv"][1 - 1].view(
            -1, 1, 1, 1
        )  # G invS4 invS3 invS1
        GinvS431G_T = torch.tensordot(
            GinvS431h, GinvS431h, dims=[[0, 2, 3], [0, 2, 3]]
        )  # for S2
        self.update_S(
            G,
            state,
        ...
        diag1 = torch.mean(
            (GinvS43Q2Q1 * state["eigen_sqrt_inv"][2 - 1].view(1, -1, 1, 1)) ** 2,
            dim=(1, 2, 3),
        )  # torch.diag( Q1.T @ GinvS432G_T @ Q1 )/(d2*d3*d4) #for diag(S1)
        self.update_eigen_value(
            state, diag1, 1 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )

        diag2 = torch.mean(
            (GinvS43Q2Q1 * state["eigen_sqrt_inv"][1 - 1].view(-1, 1, 1, 1)) ** 2,
            dim=(0, 2, 3),
        )  # torch.diag( Q2.T @ GinvS431G_T @ Q2 )/(d1*d3*d4) #for diag(S2)
        self.update_eigen_value(
            state, diag2, 2 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )
    ...
    def update_3d_preconditioner(
        ...
        GinvS1Q2Q3 = torch.einsum(
            "pqa,am->pqm", GinvS1Q2, state["Q"][3 - 1]
        )  # G invS1 Q2 Q3

        GinvS12h = GinvS1Q2 * state["eigen_sqrt_inv"][2 - 1].view(
            1, -1, 1
        )  # G invS1 invS2
        GinvS12G_T = torch.tensordot(
            GinvS12h, GinvS12h, dims=[[0, 1], [0, 1]]
        )  # for S3
        self.update_S(
            G,
            state,
        ...
        GinvS13h = GinvS1Q3 * state["eigen_sqrt_inv"][3 - 1].view(
            1, 1, -1
        )  # G invS1 invS3
        GinvS13G_T = torch.tensordot(
            GinvS13h, GinvS13h, dims=[[0, 2], [0, 2]]
        )  # for S2
        self.update_S(
            G,
            state,
        ...
        diag3 = torch.mean(
            (GinvS1Q2Q3 * state["eigen_sqrt_inv"][2 - 1].view(1, -1, 1)) ** 2,
            dim=(0, 1),
        )  # torch.diag( Q3.T @ GinvS12G_T @ Q3 )/(d1*d2) #for diag(S3)
        self.update_eigen_value(
            state, diag3, 3 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )

        diag2 = torch.mean(
            (GinvS1Q2Q3 * state["eigen_sqrt_inv"][3 - 1].view(1, 1, -1)) ** 2,
            dim=(0, 2),
        )  # torch.diag( Q2.T @ GinvS13G_T @ Q2 )/(d1*d3) #for diag(S2)
        self.update_eigen_value(
            state, diag2, 2 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )
        ...
        GinvS3Q2Q1 = torch.einsum(
            "iqm,ip->pqm", GinvS3Q2, state["Q"][1 - 1]
        )  # G invS3 Q2 Q1

        GinvS32h = GinvS3Q2 * state["eigen_sqrt_inv"][2 - 1].view(
            1, -1, 1
        )  # G invS3 invS2
        GinvS32G_T = torch.tensordot(
            GinvS32h, GinvS32h, dims=[[1, 2], [1, 2]]
        )  # for S1
        self.update_S(
            G,
            state,
        ...
        diag1 = torch.mean(
            (GinvS3Q2Q1 * state["eigen_sqrt_inv"][2 - 1].view(1, -1, 1)) ** 2,
            dim=(1, 2),
        )  # torch.diag( Q1.T @ GinvS32G_T @ Q1 )/(d2*d3) #for diag(S1)
        self.update_eigen_value(
            state, diag1, 1 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )
    ...
    def update_2d_preconditioner(
        ...
        self.update_eigen_value(
            state, ldiag, 1 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )
        self.update_eigen_value(
            state, rdiag, 2 - 1, beta_keep_Ev, beta_upd_Ev, traces, total_trace, damping
        )
            ...
            if group["use_kls"]:
                ...
                for p in group["params"]:
                    ...
                    if is_merged:
                        ...
                        grad = torch.stack(
                            [k.grad.to(dtype=self.cast_dtype) for k in group["params"]]
                        )
                    ...
                    state["step"] += 1
                    beta_keep_grad, beta_upd_grad = self._get_ema_weights(
                        state, group["betas"][0], "grad"
                    )
                    ...
                    if len(grad.shape) == 2:
                        precond_grad = (
                            grad_projected * state["eigen_sqrt_inv"][0].view(-1, 1)
                        ) * state["eigen_sqrt_inv"][1].view(1, -1)
                    elif len(grad.shape) == 3:
                        precond_grad = (
                            (grad_projected * state["eigen_sqrt_inv"][0].view(-1, 1, 1))
                        ...
                        precond_grad = (
                            (
                                grad_projected
                                * state["eigen_sqrt_inv"][0].view(-1, 1, 1, 1)
                            )
                            * state["eigen_sqrt_inv"][1].view(1, -1, 1, 1)
                            * state["eigen_sqrt_inv"][2].view(1, 1, -1, 1)
                            * state["eigen_sqrt_inv"][3].view(1, 1, 1, -1)
                        )
                    else:
                        assert False
                    ...
                    if is_merged:
                        for idx in range(total_idx):
                            group["params"][idx].add_(
                                norm_grad[idx, :, :].view(group["params"][idx].shape),
                                alpha=-step_size,
                            )
                    else:
    ...
    def get_orthogonal_matrix(self, mat, update_GG=False):
        ...
        for idx, m in enumerate(matrix):
            ...
            Q = torch.flip(Q, [1])
            v = (
                torch.ones(Q.shape[0], device=Q.device, dtype=Q.dtype)
                * self.init_factor
            )

            sqrt_inv_d = (1.0 / torch.sqrt(v)).nan_to_num_(
                nan=0.0, posinf=0.0, neginf=0.0
            )
            if not float_data:
