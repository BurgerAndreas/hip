    ...
    for optimizer, opt_state in zip(self.optimizers, optimizer_states):
        ...
        if len(optimizer.param_groups) != len(opt_state["param_groups"]):
            ...
            if model is not None and "state_dict" in checkpoint:
                ...
                id_map = {}
                for i in range(
                    min(len(opt_state["param_groups"]), len(optimizer.param_groups))
                ):
                    ...
                    if len(old_params) > len(new_param_tensors):
                        unmatched_in_checkpoint += len(old_params) - len(
                            new_param_tensors
                        )
                    else:
                ...
                if len(opt_state["param_groups"]) > len(optimizer.param_groups):
                    for i in range(
                        len(optimizer.param_groups), len(opt_state["param_groups"])
                    ):
                        unmatched_in_checkpoint += len(
                            opt_state["param_groups"][i]["params"]
                        )
                else:
                    for i in range(
                        len(opt_state["param_groups"]), len(optimizer.param_groups)
                    ):
