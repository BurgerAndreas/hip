...
if [[ "${PREP_ORCA_INPUTS:-0}" == "1" ]]; then
  python "$REPO_ROOT/scripts/prep_orca_hessian_inputs.py" \
    --repo-root "$REPO_ROOT" \
    --scratch-root "$SCRATCH_ROOT" \
    --nprocs "$NPROCS" \
    --maxcore-mb "$MAXCORE_MB" \
    --quiet
elif [[ ! -f "$SCRATCH_ROOT/jobs.tsv" ]]; then
...
for line in reversed(path.read_text(errors="replace").splitlines()):
    if line.strip():
        raise SystemExit(0 if line.strip() == "$end" else 1)
raise SystemExit(1)
PY
}
...
for row in rows:
    print("\t".join(row[key] for key in ("name", "workdir", "input_path", "output_path", "hessian_path")))
PY
}

while IFS=$'\t' read -r name workdir input_path output_path hessian_path; do
  if [[ -z "$name" ]]; then
    continue
  fi

  if complete_hessian "$hessian_path" && [[ "${OVERWRITE:-0}" != "1" ]]; then
    echo "Skipping $name because $hessian_path already exists. Set OVERWRITE=1 to rerun."
  else
    echo "Running ORCA for $name in $workdir"
    (cd "$workdir" && "$ORCA_BIN" "$input_path" > "$output_path")
  fi

  if ! complete_hessian "$hessian_path"; then
    echo "ORCA did not produce a complete Hessian at $hessian_path" >&2
    exit 1
  fi

  python "$REPO_ROOT/scripts/collect_orca_hessian_results.py" \
    --scratch-root "$SCRATCH_ROOT" \
    --sample-name "$name" || echo "Warning: HDF5 collection failed for $name; Hessian is present at $hessian_path" >&2
done < <(select_jobs)
