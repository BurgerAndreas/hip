...
def load_atomization_energies(dataset_path, dataset_name, max_samples=10000):
    ...
    print(f"  Atomization energy range: [{df['atomization_energy'].min():.4f}, {df['atomization_energy'].max():.4f}] eV")
...
def plot_atomization_energies(df_t1x, df_rgd1, output_dir="plots/eval_horm"):
    ...
    ax1.plot(x_line_t1x, p_t1x(x_line_t1x), "b--", alpha=0.7, linewidth=2, label=f"T1x fit: y={z_t1x[0]:.3f}x+{z_t1x[1]:.3f}")
    ...
    ax1.plot(x_line_rgd1, p_rgd1(x_line_rgd1), "r--", alpha=0.7, linewidth=2, label=f"RGD1 fit: y={z_rgd1[0]:.3f}x+{z_rgd1[1]:.3f}")
    ...
    for name, df in [("T1x Train", df_t1x), ("RGD1", df_rgd1)]:
        ...
        print(f"  Linear fit: y = {z_t1x[0] if name=='T1x Train' else z_rgd1[0]:.6f} * natoms + {z_t1x[1] if name=='T1x Train' else z_rgd1[1]:.6f}")
    
    # Per-atom statistics
    df_t1x_per_atom = df_t1x.copy()
    df_t1x_per_atom["atomization_energy_per_atom"] = df_t1x_per_atom["atomization_energy"] / df_t1x_per_atom["natoms"]
    
    df_rgd1_per_atom = df_rgd1.copy()
    df_rgd1_per_atom["atomization_energy_per_atom"] = df_rgd1["atomization_energy"] / df_rgd1["natoms"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compare atomization energies between T1x and RGD1")
    parser.add_argument(
        "--t1x_path",
        type=str,
    ...
    # Load datasets
    df_t1x = load_atomization_energies(args.t1x_path, "T1x Train", max_samples=args.max_samples)
    df_rgd1 = load_atomization_energies(args.rgd1_path, "RGD1", max_samples=args.max_samples)
