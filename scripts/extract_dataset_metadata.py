...
def compare_formulas_between_datasets(train_csv_path, val_csv_path, output_file=None):
    ...
    unique_train_indices = df_train[df_train['formula'].isin(unique_to_train)]['index'].tolist()
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"Percentage of training samples with unique formulas: {len(unique_train_indices)/len(df_train)*100:.2f}%\n\n")
...
def extract_dataset_metadata(lmdb_path, max_samples=None, output_file=None):
    ...
    print(f"Number of atoms range: {df_metadata['natoms'].min()} - {df_metadata['natoms'].max()}")
...
def main():
    ...
    if args.compare:
        ...
        # Compare formulas between datasets
        unique_indices, common_formulas, unique_formulas = compare_formulas_between_datasets(
