...
@pytest.mark.parametrize(
    "rngstate,length", [(0, 1), (1, 2), (2, 10), (3, 1024), (4, 0)]
)
def test_normalize(rngstate, length):
    ...
    if length > 0:
        assert abs(np.linalg.norm(x) - 1.0) < 1e-14


@pytest.mark.parametrize(
    "rngstate,length,scale",
    [(0, 1, 1.0), (1, 3, 0.5), (2, 100, 4.0), (3, 10, 0.0), (4, 0, 1.0)],
)
...
@pytest.mark.parametrize(
    "rngstate,n,m", [(0, 1, 1), (1, 5, 5), (2, 3, 7), (3, 8, 4), (6, 100, 100)]
)
...
@pytest.mark.parametrize("rngstate,scale", [(0, 1.0), (1, 0.1), (2, 100.0)])
...
@pytest.mark.parametrize("rngstate,n,mx,my", [(2, 10, 2, 4)])
def test_mgs(rngstate, n, mx, my):
    ...
    assert my2 >= 0
    np.testing.assert_allclose(Y[:, :my2].T @ Y[:, :my2], np.eye(my2), atol=1e-10)
    X = get_matrix(n, mx, rng=rng)
    mx2 = wrappers["mgs"](X, Y)
    assert mx2 >= 0
    np.testing.assert_allclose(X[:, :mx2].T @ X[:, :mx2], np.eye(mx2), atol=1e-10)
    np.testing.assert_allclose(
        X[:, :mx2].T @ Y[:, :my2], np.zeros((mx2, my2)), atol=1e-10
    )
