...
@pytest.mark.parametrize(
    "dim,order,eta,threepoint", [(10, 4, 1e-6, True), (10, 4, 1e-6, False)]
)
def test_exact(dim, order, eta, threepoint):
    ...
    H = NumericalHessian(
        lambda x: poly(x)[:2], g0=g, x0=x, eta=eta, threepoint=threepoint
    )
...
@pytest.mark.parametrize(
    "dim,order,eta,threepoint,gamma,method,maxiter",
    [
        (10, 4, 1e-6, False, 0.0, "jd0", None),
...
def test_rayleigh_ritz(dim, order, eta, threepoint, gamma, method, maxiter):
    ...
    l1, V1, AV1 = rayleigh_ritz(H, gamma, np.eye(dim), method=method, maxiter=maxiter)
